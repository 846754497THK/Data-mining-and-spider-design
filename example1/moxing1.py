...
y_pred1 = clf0.predict(vec.transform(X_test))
...
#混淆矩阵
print('混淆矩阵：'+str(metrics.confusion_matrix(y_test,y_pred1)))
#
...
x_pre=pd.concat([X_test,y_pred1],axis=1)
...
print('随机森林实际分类与预测分类词云图（正面）：\n')
word_cloud('正面1(实际)',cut_text1)
word_cloud('正面1(预测)',cut_text2)

print('随机森林实际分类与预测分类词云图（中性）：\n')
word_cloud('中性1(实际)',cut_text3)
word_cloud('中性1(预测)',cut_text4)

print('随机森林实际分类与预测分类词云图（负面）：\n')
word_cloud('负面1(实际)',cut_text5)
word_cloud('负面1(预测)',cut_text6)
